...
const TableComponent: React.FC<TableComponentProps> = ({
  ...
  const [positionsWithDates, setPositionsWithDates] = useState<
  ...
  useEffect(() => {
    const fetchPoolInfo = async (
      ...
      try {
        const response = await fetch(
          `https://dlmm-api.meteora.ag/pair/${pubkey}`
        );
    ...
    const fetchAllPoolInfo = async () => {
      ...
      for (const [key, value] of Array.from(dataMap.entries())) {
        const info = await fetchPoolInfo(
          key,
          value.tokenXDecimal,
          value.tokenYDecimal
        );
    ...
    const fetchPositionDates = async () => {
      ...
      for (const [key, value] of Array.from(dataMap.entries())) {
        const positionsWithDate = value.lbPairPositionsData.map(
          (position: PositionData) => position
        );

        positionsWithDate.sort(
        ...
        const newestDate =
          positionsWithDate.length > 0
            ? positionsWithDate[0].lastUpdatedAt
            : new Date(0);
      ...
      const sortedTokenGroups = Array.from(tokenGroups.values()).map(
        (group) => {
          ...
          group.pairs.sort(
            (a, b) =>
              b.pools[0].newestDate.getTime() - a.pools[0].newestDate.getTime()
          );
          return group;
        }
      );
      ...
      // Sort token groups
      sortedTokenGroups.sort(
        (a, b) =>
          b.pairs[0].pools[0].newestDate.getTime() -
          a.pairs[0].pools[0].newestDate.getTime()
      );
  ...
  const createTableRows = (): React.ReactNode[] => {
    return groupedPools.map((tokenGroup, tokenGroupIndex) => {
      const allPositionsInGroup = tokenGroup.pairs.flatMap((pair) =>
        pair.pools.flatMap(
          (pool) =>
            positionsWithDates
              .get(pool.key)
              ?.map((pos) => pos.publicKey.toString()) || []
        )
      );
      ...
      return (
        <div key={`tokenGroup-${tokenGroupIndex}`} className="mb-4">
          <div
            className={`border border-base-300 rounded-lg bg-base-100 p-4 ${
              isTokenGroupSelected ? "bg-light-green" : ""
            }`}
          >
            <div
              className={`text-xl font-medium flex items-center justify-between mb-4 cursor-pointer p-2 rounded
                            ${
                              isTokenGroupSelected
                                ? "bg-light-green"
                                : "hover-light-green"
                            }`}
            ...
            <div>
              {tokenGroup.pairs.map((pair, pairIndex) => {
                const pairPositions = pair.pools.flatMap(
                  (pool) =>
                    positionsWithDates
                      .get(pool.key)
                      ?.map((pos) => pos.publicKey.toString()) || []
                ...
                return (
                  <div
                    key={`pair-${tokenGroupIndex}-${pairIndex}`}
                    className={`mt-4 first:mt-0 ${
                      isPairSelected ? "bg-light-green" : ""
                    }`}
                  >
                    ...
                    {pair.pools.map(({ key, value }) => {
                      ...
                      const dailyYield =
                        poolInfo?.liquidity && poolInfo?.fees_24h
                          ? (poolInfo.fees_24h /
                              parseFloat(poolInfo.liquidity)) *
                            100
                          : 0;

                      return (
                        <div key={`${key}-header`} className="mb-4">
                          <div className="bg-base-200 shadow-sm rounded-lg p-3 w-full mx-auto">
                            ...
                            <div className="flex flex-wrap justify-between items-center mb-2">
                              ...
                              <div className="text-sm font-medium text-gray-600">
                                {value.nameY} per {value.nameX}{" "}
                                {formatPrice(poolInfo?.current_price || 0)}
                              </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                              <div>
                                ...
                                <div className="font-semibold">
                                  {formatCurrency(poolInfo?.liquidity || 0)}
                                </div>
                              </div>
                              <div>
                                <div className="text-xs text-gray-500">
                              ...
                              <div>
                                ...
                                <div className="font-semibold">
                                  {formatCurrency(poolInfo?.fees_24h || 0)}
                                </div>
                              </div>
                              <div>
                                <div className="text-xs text-gray-500">
                                ...
                                <div className="font-semibold">
                                  {prettifyNumber(dailyYield, true)}%
                                </div>
                              </div>
                            </div>

                            <div className="mt-2 relative h-5 bg-gray-100 rounded-full overflow-hidden">
                              <div
                                className="absolute inset-y-0 left-0 bg-gradient-to-r from-yellow-200 to-red-600 transition-all duration-500 ease-out"
                                style={{
                                  width: `${poolInfo?.liquidityDistribution.percentX}%`,
                                }}
                              />
                              <div
                                ...
                                style={{
                                  width: `${poolInfo?.liquidityDistribution.percentY}%`,
                                }}
                              />
                              <div className="absolute inset-0 flex items-center justify-center text-xs font-medium text-gray-700">
                                {value.nameX}{" "}
                                {prettifyNumber(
                                  poolInfo?.liquidityDistribution.percentX
                                )}
                                % / {value.nameY}{" "}
                                {prettifyNumber(
                                  poolInfo?.liquidityDistribution.percentY
                                )}
                                %
                              </div>
                            </div>
                            ...
                            <details>
                              <summary className="cursor-pointer text-sm font-medium mt-2">
                                Pool Settings
                              </summary>
                                ...
                                <tbody>
                                  {positions.map((position, index) => (
                                    ...
                                    >
                                      ...
                                      <td className="text-right px-2 py-2 border-r whitespace-nowrap">
                                        <div>
                                          {prettifyNumber(position.feeX)}
                                        </div>
                                        <div>
                                          {prettifyNumber(position.feeY)}
                                        </div>
                                      </td>
                                      <td className="text-right px-2 py-2 border-r whitespace-nowrap">
                                        <div>
                                          {prettifyNumber(
                                            position.claimedFeeX || 0
                                          )}
                                        </div>
                                        <div>
                                          {prettifyNumber(
                                            position.claimedFeeY || 0
                                          )}
                                        </div>
                                      </td>
                                      <td className="px-2 py-2 border-r w-[300px]">
                                        {position.lowerBinId !== undefined &&
                                        position.upperBinId !== undefined &&
                                        value.activeBin !== undefined ? (
                                          <div
                                          ...
                                          >
                                            <RangeIndicator
                                              ...
                                              mintXDigits={
                                                poolInfo?.tokenXDecimal || 0
                                              }
                                              mintYDigits={
                                                poolInfo?.tokenYDecimal || 0
                                              }
                                            />
                                          </div>
                                        ) : (
    ...
    `}
                                ...
                                >
                                  <div className="mb-2 text-sm text-base-content opacity-70">
                                    Last Updated:{" "}
                                    {formatDistanceToNow(
                                      position.lastUpdatedAt,
                                      {
                                  ...
                                  <div className="grid grid-cols-2 gap-2">
                                    <div>
                                      ...
                                      <div>
                                        {prettifyNumber(position.totalXAmount)}{" "}
                                        {value.nameX}
                                      </div>
                                      <div>
                                        {prettifyNumber(position.totalYAmount)}{" "}
                                        {value.nameY}
                                      </div>
                                    </div>
                                    <div>
  ...
  return <div>{createTableRows()}</div>;
};
