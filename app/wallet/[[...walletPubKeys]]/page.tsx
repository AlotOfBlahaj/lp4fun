...
const createDataMap = async (
  wallet: string
): Promise<Map<string, PoolData>> => {
  const connection = new Connection(config.RPC_ENDPOINT);
  const user = new PublicKey(wallet);
  const positions = await fetchWithRetry(() =>
    DLMM.getAllLbPairPositionsByUser(connection, user)
  );
  const map = new Map<string, PoolData>();
  await Promise.all(
    Array.from(positions.entries()).map(async ([key, position]) => {
      const tokenInfo = await fetchTokenPrice(
        position.tokenX.publicKey,
        position.tokenY.publicKey
      );

      const lbPairPositionsData = position.lbPairPositionsData.map(
        (pos: LbPosition): PositionData => {
          const claimedFeeXAmount =
            pos.positionData.totalClaimedFeeXAmount.toString();
          const claimedFeeYAmount =
            pos.positionData.totalClaimedFeeYAmount.toString();

          return {
            lastUpdatedAt: bnToDate(pos.positionData.lastUpdatedAt),
            totalXAmount: formatTokenBalance(
              BigInt(pos.positionData.totalXAmount.split(".")[0]),
              position.tokenX.decimal
            ),
            totalYAmount: formatTokenBalance(
              BigInt(pos.positionData.totalYAmount.split(".")[0]),
              position.tokenY.decimal
            ),
            feeX: formatTokenBalance(
              BigInt(pos.positionData.feeX.toString()),
              position.tokenX.decimal
            ),
            feeY: formatTokenBalance(
              BigInt(pos.positionData.feeY.toString()),
              position.tokenY.decimal
            ),
            ...
            claimedFeeXAmount,
            claimedFeeYAmount,
            claimedFeeX: formatTokenBalance(
              BigInt(claimedFeeXAmount),
              position.tokenX.decimal
            ),
            claimedFeeY: formatTokenBalance(
              BigInt(claimedFeeYAmount),
              position.tokenY.decimal
            ),
          };
        }
      );
      const mintInfoX = await getTokenMetadata(
        connection,
        position.tokenX.publicKey
      );
      const mintInfoY = await getTokenMetadata(
        connection,
        position.tokenY.publicKey
      );
      map.set(key, {
...
const WalletPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (wallets.length === 0) {
        setError(
          "No wallets specified. Please provide at least one wallet address."
        );
        setIsLoading(false);
        return;
      }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen text-center">
        <div className="text-2xl font-bold mb-4">
          No open DLMM positions found
        </div>
        <div className="text-lg mb-6">
          Looks like you haven&apos;t dipped your toes in the DLMM pool yet!
        </div>
  ...
  return (
    <>
      <div className="container mx-auto mb-16">
        {" "}
        {/* Added margin-bottom to prevent footer overlap */}
          ...
          <div className="container mx-auto px-4 py-3 flex justify-center items-center space-x-4">
            ...
            <button
              ...
              className={`btn btn-sm ${
                selectedPositions.size > 0 ? "btn-primary" : "btn-disabled"
              }`}
